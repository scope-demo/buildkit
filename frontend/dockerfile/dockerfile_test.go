...
`)
	...
	for _, x := range []struct {
		...
		testutil.GetTracedTest(t).Run(x.name, func(t *testing.T) {
...
`)
	...
	for i, exp := range []struct {
		...
		testutil.GetTracedTest(t).Run(exp.p, func(t *testing.T) {
...
`)
	...
	du2, err := c.DiskUsage(testutil.GetContext(t))
	require.NoError(t, err)
...
`)
	...
	_, err = c.Solve(testutil.GetContext(t), def, client.SolveOpt{
		Exports: []client.ExportEntry{
			{
				Type:      client.ExporterLocal,
				OutputDir: destDir,
...
`)
	...
	for i := 0; i < 2; i++ {
		err = c.Prune(testutil.GetContext(t), nil, client.PruneAll)
		require.NoError(t, err)
...
`)
	...
	_, err = f.Solve(testutil.GetContext(t), c, opt, nil)
	require.NoError(t, err)
...
`)
	...
	_, err = c.Build(testutil.GetContext(t), client.SolveOpt{
		Exports: []client.ExportEntry{
			{
				Type:      client.ExporterLocal,
				OutputDir: destDir,
