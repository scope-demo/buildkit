...
func TestChecksumSymlinkNoParentScan(t *testing.T) {
	...
	dgst, err := cc.Checksum(scopeagent.GetContextFromTest(t), ref, "aa/ln/bb/cc/dd", true)
	require.NoError(t, err)
...
func TestChecksumHardlinks(t *testing.T) {
	...
	dgst, err := cc.Checksum(scopeagent.GetContextFromTest(t), ref, "abc/foo", false)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgst)

	dgst, err = cc.Checksum(scopeagent.GetContextFromTest(t), ref, "ln", false)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgst)

	dgst, err = cc.Checksum(scopeagent.GetContextFromTest(t), ref, "ln2", false)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgst)
	...
	dgst, err = cc2.Checksum(scopeagent.GetContextFromTest(t), ref, "abc/foo", false)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgst)

	dgst, err = cc2.Checksum(scopeagent.GetContextFromTest(t), ref, "ln", false)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgst)

	dgst, err = cc2.Checksum(scopeagent.GetContextFromTest(t), ref, "ln2", false)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgst)
...
func TestChecksumWildcard(t *testing.T) {
	...
	dgst, err := cc.ChecksumWildcard(scopeagent.GetContextFromTest(t), ref, "f*o", false)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgst)

	expFoos := digest.Digest("sha256:c9f914ad7ad8fe6092ce67484b43ca39c2087aabf9e4a1b223249b0f8b09b9f2")

	dgst, err = cc.ChecksumWildcard(scopeagent.GetContextFromTest(t), ref, "f*", false)
	require.NoError(t, err)
	require.Equal(t, expFoos, dgst)

	dgst, err = cc.ChecksumWildcard(scopeagent.GetContextFromTest(t), ref, "x/d?", false)
	require.NoError(t, err)
	require.Equal(t, dgstDirD0, dgst)

	dgst, err = cc.ChecksumWildcard(scopeagent.GetContextFromTest(t), ref, "x/d?/def", true)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgst)

	dgst, err = cc.ChecksumWildcard(scopeagent.GetContextFromTest(t), ref, "y*", true)
	require.NoError(t, err)
	require.Equal(t, expFoos, dgst)

	err = ref.Release(scopeagent.GetContextFromTest(t))
	require.NoError(t, err)
}
...
func TestSymlinksNoFollow(t *testing.T) {
	...
	dgst, err := cc.ChecksumWildcard(scopeagent.GetContextFromTest(t), ref, "sym", false)
	require.NoError(t, err)
	require.Equal(t, expectedSym, dgst)

	dgst, err = cc.ChecksumWildcard(scopeagent.GetContextFromTest(t), ref, "sym2", false)
	require.NoError(t, err)
	require.NotEqual(t, expectedSym, dgst)

	dgst, err = cc.ChecksumWildcard(scopeagent.GetContextFromTest(t), ref, "foo/ghi", false)
	require.NoError(t, err)
	require.Equal(t, expectedSym, dgst)

	_, err = cc.ChecksumWildcard(scopeagent.GetContextFromTest(t), ref, "foo/ghi", true) // same because broken symlink
	require.Error(t, err)
	require.Equal(t, errNotFound, errors.Cause(err))

	_, err = cc.ChecksumWildcard(scopeagent.GetContextFromTest(t), ref, "y1", true)
	require.Error(t, err)
	require.Equal(t, errNotFound, errors.Cause(err))

	dgst, err = cc.Checksum(scopeagent.GetContextFromTest(t), ref, "sym", false)
	require.NoError(t, err)
	require.Equal(t, expectedSym, dgst)

	dgst, err = cc.Checksum(scopeagent.GetContextFromTest(t), ref, "foo/ghi", false)
	require.NoError(t, err)
	require.Equal(t, expectedSym, dgst)
...
func TestChecksumBasicFile(t *testing.T) {
	...
	_, err = cc.Checksum(scopeagent.GetContextFromTest(t), ref, "nosuch", true)
	require.Error(t, err)

	dgst, err := cc.Checksum(scopeagent.GetContextFromTest(t), ref, "foo", true)
	require.NoError(t, err)

	require.Equal(t, dgstFileData0, dgst)

	// second file returns different hash
	dgst, err = cc.Checksum(scopeagent.GetContextFromTest(t), ref, "bar", true)
	require.NoError(t, err)
	...
	dgst, err = cc.Checksum(scopeagent.GetContextFromTest(t), ref, "d0/abc", true)
	require.NoError(t, err)
	...
	dgst, err = cc.Checksum(scopeagent.GetContextFromTest(t), ref, "d0/def", true)
	require.NoError(t, err)

	require.Equal(t, dgstFileData0, dgst)

	_, err = cc.Checksum(scopeagent.GetContextFromTest(t), ref, "d0/ghi", true)
	require.Error(t, err)
	require.Equal(t, errNotFound, errors.Cause(err))

	dgst, err = cc.Checksum(scopeagent.GetContextFromTest(t), ref, "/", true)
	require.NoError(t, err)

	require.Equal(t, digest.Digest("sha256:427c9cf9ae98c0f81fb57a3076b965c7c149b6b0a85625ad4e884236649a42c6"), dgst)

	dgst, err = cc.Checksum(scopeagent.GetContextFromTest(t), ref, "d0", true)
	require.NoError(t, err)
	...
	dgst, err = cc.Checksum(scopeagent.GetContextFromTest(t), ref, "abc/aa/foo", true)
	require.NoError(t, err)
...
func TestHandleChange(t *testing.T) {
	...
	dgstFoo, err := cc.Checksum(scopeagent.GetContextFromTest(t), ref, "foo", true)
	require.NoError(t, err)
	...
	_, err = cc.Checksum(scopeagent.GetContextFromTest(t), ref, "d0", true)
	require.Error(t, err)
	require.Equal(t, errNotFound, errors.Cause(err))

	_, err = cc.Checksum(scopeagent.GetContextFromTest(t), ref, "d0/abc", true)
	require.Error(t, err)
	require.Equal(t, errNotFound, errors.Cause(err))
...
func TestHandleRecursiveDir(t *testing.T) {
	...
	dgst, err := cc.Checksum(scopeagent.GetContextFromTest(t), ref, "d0/foo/bar", true)
	require.NoError(t, err)
	...
	dgst2, err := cc.Checksum(scopeagent.GetContextFromTest(t), ref, "d1", true)
	require.NoError(t, err)
	require.Equal(t, dgst2, dgst)

	_, err = cc.Checksum(scopeagent.GetContextFromTest(t), ref, "", true)
	require.NoError(t, err)
}
...
func TestChecksumUnorderedFiles(t *testing.T) {
	...
	dgst2, err := cc.Checksum(scopeagent.GetContextFromTest(t), ref, "d0", true)
	require.NoError(t, err)
...
func TestSymlinkInPathScan(t *testing.T) {
	...
	dgst, err := Checksum(scopeagent.GetContextFromTest(t), ref, "d0/def/foo", true)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgst)
...
func TestSymlinkNeedsScan(t *testing.T) {
	...
	_, err = Checksum(scopeagent.GetContextFromTest(t), ref, "d0/d1", true)
	require.NoError(t, err)

	dgst, err := Checksum(scopeagent.GetContextFromTest(t), ref, "d0/d1/def/foo", true)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgst)
...
func TestSymlinkAbsDirSuffix(t *testing.T) {
	...
	dgst, err := Checksum(scopeagent.GetContextFromTest(t), ref, "link/foo", true)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgst)
...
func TestSymlinkThroughParent(t *testing.T) {
	...
	dgst, err := Checksum(scopeagent.GetContextFromTest(t), ref, "link1/sub/foo", true)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgst)

	dgst, err = Checksum(scopeagent.GetContextFromTest(t), ref, "link2/sub/foo", true)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgst)

	dgst, err = Checksum(scopeagent.GetContextFromTest(t), ref, "link3/sub/foo", true)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgst)

	dgst, err = Checksum(scopeagent.GetContextFromTest(t), ref, "link4/sub/foo", true)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgst)

	dgst, err = Checksum(scopeagent.GetContextFromTest(t), ref, "link5/sub/foo", true)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgst)

	dgst, err = Checksum(scopeagent.GetContextFromTest(t), ref, "link1/sub/link/sub/foo", true)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgst)
...
func TestSymlinkInPathHandleChange(t *testing.T) {
	...
	dgst, err := cc.Checksum(scopeagent.GetContextFromTest(t), ref, "d1/def/foo", true)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgst)

	dgst, err = cc.Checksum(scopeagent.GetContextFromTest(t), ref, "d1/def/bar/abc", true)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgst)

	dgstFileData0, err := cc.Checksum(scopeagent.GetContextFromTest(t), ref, "sub/d0", true)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgstDirD0)

	dgstFileData0, err = cc.Checksum(scopeagent.GetContextFromTest(t), ref, "d1/def/baz", true)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgstDirD0)

	dgstFileData0, err = cc.Checksum(scopeagent.GetContextFromTest(t), ref, "d1/def/bay", true)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgstDirD0)

	dgstFileData0, err = cc.Checksum(scopeagent.GetContextFromTest(t), ref, "link", true)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgstDirD0)
...
func TestPersistence(t *testing.T) {
	...
	dgst, err := Checksum(scopeagent.GetContextFromTest(t), ref, "foo", true)
	require.NoError(t, err)
	require.Equal(t, dgstFileData0, dgst)
	...
	ref, err = cm.Get(scopeagent.GetContextFromTest(t), id)
	require.NoError(t, err)
...
func createRef(t *testing.T, cm cache.Manager, files []string) cache.ImmutableRef {
	mref, err := cm.New(scopeagent.GetContextFromTest(t), nil, cache.CachePolicyRetain)
	require.NoError(t, err)

	mounts, err := mref.Mount(scopeagent.GetContextFromTest(t), false)
	require.NoError(t, err)
	...
	ref, err := mref.Commit(scopeagent.GetContextFromTest(t))
	require.NoError(t, err)
